...
// Ключи интересов — не зависят от языка, сохраняются в SharedPreferences
const List<String> interestKeys = [
  'it',
  'medicine',
  'business',
  'grants',
...
class _InterestQuizScreenState extends State<InterestQuizScreen> {
  ...
  // Подгружаем ранее выбранные интересы из SharedPreferences
  Future<void> _loadSavedInterests() async {
    final prefs = await SharedPreferences.getInstance();
    final saved = prefs.getStringList(kInterestsPrefsKey) ?? [];
    setState(() {
      for (int i = 0; i < interestKeys.length; i++) {
        if (saved.contains(interestKeys[i])) _selected.add(i);
      }
    });
  }
  ...
  Future<void> _proceed() async {
    ...
    try {
      final prefs = await SharedPreferences.getInstance();
      final selectedKeys = _selected.map((i) => interestKeys[i]).toList();
      await prefs.setStringList(kInterestsPrefsKey, selectedKeys);
    } finally {
