...
class GlobalFavoritesNotifier extends ValueNotifier<Set<String>> {
  ...
  /// Возвращает `true`, если id был добавлен,
  /// и `false` — если удалён.
  ///
  /// Бросает [NotAuthenticatedException]
  /// для незалогиненных.
  Future<bool> toggle(
    String universityId,
  ) async {
    ...
    final wasFavorite =
        value.contains(universityId);

    value = await _repository.toggle(
      universityId,
    );
  ...
  /// Очищает все лайки локально.
  /// Вызывается при выходе из аккаунта
  /// и при удалении аккаунта.
  void clear() {
    value = <String>{};
    notifyListeners();
  }
