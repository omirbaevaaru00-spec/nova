...
class UniversityDetailCubit extends Cubit<UniversityDetailState> {
  ...
  /// Загружает основные данные университета.
  ///
  /// [initialTab] — если передан, сразу переключает на нужную вкладку
  /// и подгружает её данные (например, сразу открыть новости).
  Future<void> load(String id, {UniversityDetailTab? initialTab}) async {
    emit(state.copyWith(status: UniversityDetailStatus.loading));
    try {
      final uni = await _university.getById(id);
      if (uni == null) {
      ...
      final tab = initialTab ?? UniversityDetailTab.description;
      emit(state.copyWith(
        status: UniversityDetailStatus.ready,
        university: uni,
        tab: tab,
      ...
      // Если начальная вкладка не description — сразу подгружаем данные
      if (tab == UniversityDetailTab.programs) {
        await _loadPrograms(id);
      } else if (tab == UniversityDetailTab.news) {
        await _loadNews(id);
      }
    } catch (e, st) {
      _log.e('load($id) failed', error: e, stackTrace: st);
