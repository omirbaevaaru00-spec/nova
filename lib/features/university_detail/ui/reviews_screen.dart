...
class _ReviewsView extends StatelessWidget {
        ...
        return Scaffold(
          ...
          body: () {
            ...
            return _ReviewsList(
              reviews: state.reviews,
              isDark: isDark,
              currentUserId: FirebaseService.instance.auth.currentUser?.uid,
            );
          }(),
        );
      },
  ...
  void _showAddSheet(BuildContext context, bool isDark) {
    showModalBottomSheet<void>(
      ...
      builder: (sheetCtx) => BlocProvider.value(
        value: context.read<ReviewsCubit>(),
        child: _AddReviewSheet(isDark: isDark, parentContext: context),
      ),
    );
  }
}
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                shape: BoxShape.circle,
                color: isDark ? AppColors.surfaceMutedDark : AppColors.backgroundLight,
              ),
              child: Icon(Icons.rate_review_outlined, size: 36,
...
class _ReviewsList extends StatelessWidget {
  const _ReviewsList({required this.reviews, required this.isDark, required this.currentUserId});
  final List<UniversityReview> reviews;
  final bool isDark;
  ...
  Widget build(BuildContext context) {
    return ListView.separated(
      ...
      itemBuilder: (context, i) => _ReviewCard(
        review: reviews[i],
        isDark: isDark,
        isOwner: currentUserId != null && reviews[i].authorUid == currentUserId,
      ),
    );
  }
}
...
class _ReviewCard extends StatelessWidget {
  const _ReviewCard({required this.review, required this.isDark, required this.isOwner});
  final UniversityReview review;
  final bool isDark;
  final bool isOwner;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              _Avatar(name: review.authorName, photoUrl: review.authorPhotoUrl),
              const SizedBox(width: 12),
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(review.authorName,
                        style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600,
                            color: isDark ? AppColors.textInverse : AppColors.textPrimary)),
                    const SizedBox(height: 2),
                    Text('${review.speciality} · ${review.year}',
                        style: const TextStyle(fontSize: 12, color: AppColors.textSecondary)),
                  ],
                ),
              ),
              Text(DateFormat('dd.MM.yy').format(review.createdAt),
          ...
          Text(review.text,
              style: TextStyle(fontSize: 14, height: 1.55,
                  color: isDark ? AppColors.textInverse.withValues(alpha: 0.9) : AppColors.textPrimary)),
        ],
      ),
    );
  }
...
class _DeleteButton extends StatelessWidget {
  const _DeleteButton({required this.onConfirm, required this.l10n});
        ...
        builder: (ctx) => AlertDialog(
          ...
          actions: [
            ...
            TextButton(
              ...
              child: Text(l10n.reviewDeleteConfirm,
    ...
    return CircleAvatar(
      ...
      child: photoUrl == null
          ? Text(name.isNotEmpty ? name[0].toUpperCase() : '?',
              style: const TextStyle(color: AppColors.brandAccent, fontWeight: FontWeight.w700, fontSize: 16))
          : null,
    );
  }
}
    ...
    return Row(
      children: [
        ...List.generate(5, (i) {
          final fill = math.min(1.0, math.max(0.0, rating - i));
          return Icon(
            fill >= 1.0 ? Icons.star_rounded : fill >= 0.5 ? Icons.star_half_rounded : Icons.star_outline_rounded,
...
class _AddReviewSheet extends StatefulWidget {
  const _AddReviewSheet({required this.isDark, required this.parentContext});
  final bool isDark;
...
class _AddReviewSheetState extends State<_AddReviewSheet> {
  ...
  @override
  void dispose() { _textCtrl.dispose(); _specialityCtrl.dispose(); super.dispose(); }

  bool get _canSubmit => _rating > 0 && _textCtrl.text.trim().isNotEmpty && !_isSubmitting;
  ...
  Future<void> _submit() async {
    ...
    final review = UniversityReview(
      id: DateTime.now().millisecondsSinceEpoch.toString(),
      authorUid: user?.uid ?? '',
      authorName: _resolveNickname(user),
      authorPhotoUrl: user?.photoURL,
      rating: _rating,
      text: _textCtrl.text.trim(),
      year: _year,
      speciality: _specialityCtrl.text.trim().isNotEmpty ? _specialityCtrl.text.trim() : '—',
      createdAt: DateTime.now(),
    );
    try {
      await context.read<ReviewsCubit>().submitReview(review);
    ...
    if (widget.parentContext.mounted) {
      _ReviewSuccessOverlay.show(widget.parentContext);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Container(
        decoration: BoxDecoration(
          color: widget.isDark ? AppColors.surfaceMutedDark : AppColors.backgroundLight,
          borderRadius: const BorderRadius.vertical(top: Radius.circular(28)),
        ),
        padding: const EdgeInsets.fromLTRB(24, 16, 24, 32),
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 20),
              Text(l10n.reviewsAdd,
                  style: TextStyle(fontSize: 20, fontWeight: FontWeight.w700,
                      color: widget.isDark ? AppColors.textInverse : AppColors.textPrimary)),
              const SizedBox(height: 24),
              Center(child: _AnimatedStarPicker(
                rating: _rating,
                onChanged: (r) { HapticFeedback.selectionClick(); setState(() => _rating = r); },
              )),
              AnimatedSwitcher(
                duration: const Duration(milliseconds: 200),
                child: Padding(
                  ...
                  child: Center(
                    child: _rating > 0
                        ? Text(_ratingLabel(_rating), key: ValueKey(_rating),
                            style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: AppColors.brandAccent))
                        : const Text('Коснитесь звезды для оценки',
                            style: TextStyle(fontSize: 13, color: AppColors.textMuted)),
                  ),
                ),
              ),
              const SizedBox(height: 20),
              _Field(controller: _specialityCtrl, hint: l10n.reviewSpeciality,
                  isDark: widget.isDark, onChanged: (_) => setState(() {})),
              const SizedBox(height: 12),
              _YearPicker(year: _year, isDark: widget.isDark,
                  onChanged: (y) => setState(() => _year = y), l10n: l10n),
              const SizedBox(height: 12),
              _Field(controller: _textCtrl, hint: l10n.reviewText,
                  maxLines: 4, isDark: widget.isDark, onChanged: (_) => setState(() {})),
              const SizedBox(height: 24),
              AnimatedOpacity(
                opacity: _canSubmit ? 1.0 : 0.4,
                duration: const Duration(milliseconds: 250),
...
class _AnimatedStarPicker extends StatelessWidget {
  const _AnimatedStarPicker({required this.rating, required this.onChanged});
  final double rating;
  final ValueChanged<double> onChanged;
  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: List.generate(5, (i) {
        ...
        final filled = rating >= v;
        return GestureDetector(
          onTap: () => onChanged(v),
          child: TweenAnimationBuilder<double>(
            key: ValueKey('s${i}_$filled'),
            tween: Tween(begin: filled ? 0.6 : 1.0, end: filled ? 1.25 : 1.0),
            duration: const Duration(milliseconds: 350),
            curve: Curves.elasticOut,
...
class _Field extends StatelessWidget {
  const _Field({required this.controller, required this.hint, required this.isDark,
  ...
  Widget build(BuildContext context) {
    return TextField(
      ...
      style: TextStyle(color: isDark ? AppColors.textInverse : AppColors.textPrimary, fontSize: 15),
      decoration: InputDecoration(
        hintText: hint,
        hintStyle: const TextStyle(color: AppColors.textSecondary),
        filled: true,
        fillColor: isDark ? AppColors.backgroundDark.withValues(alpha: 0.5) : AppColors.surfaceMuted,
...
class _YearPicker extends StatelessWidget {
  const _YearPicker({required this.year, required this.isDark, required this.onChanged, required this.l10n});
  final int year;
  final bool isDark;
  final ValueChanged<int> onChanged;
  final AppLocalizations l10n;
  ...
  Widget build(BuildContext context) {
    ...
    return DropdownButtonFormField<int>(
      ...
      dropdownColor: isDark ? AppColors.surfaceMutedDark : AppColors.backgroundLight,
      style: TextStyle(color: isDark ? AppColors.textInverse : AppColors.textPrimary, fontSize: 15),
...
class _ReviewSuccessOverlay {
  static void show(BuildContext context) {
    final overlay = Overlay.of(context);
    late OverlayEntry entry;
    entry = OverlayEntry(builder: (_) => _SuccessWidget(onDone: () => entry.remove()));
    overlay.insert(entry);
  }
}

class _SuccessWidget extends StatefulWidget {
  const _SuccessWidget({required this.onDone});
            ...
            child: Material(color: Colors.transparent,
              child: Container(
                ...
                decoration: BoxDecoration(
                  color: isDark ? AppColors.surfaceMutedDark : AppColors.backgroundLight,
                  borderRadius: BorderRadius.circular(20),
                  border: Border.all(color: AppColors.brandAccent.withValues(alpha: 0.3), width: 1.5),
                ...
                child: Row(
                  children: [
                    Container(
                      width: 44, height: 44,
                      decoration: BoxDecoration(color: AppColors.brandAccent.withValues(alpha: 0.15), shape: BoxShape.circle),
                      child: const Icon(Icons.check_rounded, color: AppColors.brandAccent, size: 24),
                    ),
                        ...
                        children: [
                          Text('Спасибо за ваш отзыв!',
                              style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700,
                                  color: isDark ? AppColors.textInverse : AppColors.textPrimary)),
                          const SizedBox(height: 2),
                          const Text('Ваш отзыв поможет другим абитуриентам',
